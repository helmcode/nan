...
- **The windows are the real ones.** They are measured against the proxy, and they are the same numbers the [VS Code](/docs/vscode) block carries and the cards on [Models](/docs/models) publish rounded. If you raise them by hand, OpenCode will fill the conversation up to a point where the model starts rejecting requests.
