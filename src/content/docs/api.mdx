...
Web search **does not go through LiteLLM**, so the per-key LiteLLM limits (see [Rate limits](#rate-limits)) do **not** apply to it. These limits are specific to the search endpoint and are enforced per API key:
