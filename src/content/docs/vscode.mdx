...
## Optional: the community VS Code extension

If you would rather not maintain `chatLanguageModels.json` by hand, there is a community extension that walks you through the same setup: [svg153/nan-vscode](https://github.com/svg153/nan-vscode). The manual flow below stays available as the reference path.
...
3. Run **NaN Builders: Manage Provider**, paste your NaN API key (the one starting with `sk-`) and let it fetch the models your key can use.
4. If your access changes later, run **NaN Builders: Refresh Models**.

<Callout title="Community extension" variant="info">
[svg153/nan-vscode](https://github.com/svg153/nan-vscode) is community-maintained: not an official NaN Builders product and not supported by NaN Builders. It configures **chat and agent mode** through the same Custom Endpoint, it does not touch inline completion, and it does not replace the manual instructions below.
</Callout>
